...
module Jazzy::SymbolGraph
  class Graph
    ...
    def ext_key(usr, constraints)
      usr + constraints.join
    end

    def add_ext_member(type_usr, member_node)
      key = ext_key(type_usr, member_node.symbol.constraints)
      if ext_node = ext_nodes[key]
        ext_node.add_child(member_node)
      else
        ext_nodes[key] = ExtNode.new_for_member(type_usr, member_node)
      end
    end
    ...
    def redundant_conformance?(rel, type, protocol)
      type &&
        rel.constraints.empty? &&
        type.conformance?(protocol)
    end
    ...
    def rebuild_rel(rel)
      ...
      when :memberOf
        ...
        unless target && target.try_add_child(source)
          add_ext_member(rel.target_usr, source)
        end
